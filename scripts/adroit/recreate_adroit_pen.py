...
class AdroitPenStepPreProcessor(StepPreProcessor):
    def __call__(self, env, **kwargs):
        step_data = super().__call__(env, **kwargs)

        step_data['state'] = env.get_env_state()
        # print(step_data['state']['obj_pos'])
        return step_data


env = SetInitialState(gym.make('AdroitHandPen-v1', max_episode_steps=200))
env = DataCollectorV0(env, step_preprocessor=AdroitPenStepPreProcessor, record_infos=True, max_steps_buffer=10000)
